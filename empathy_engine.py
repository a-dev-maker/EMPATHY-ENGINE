...
Usage (CLI):
    ...
    python mechanistic_empathy_engine.py -c adhd -g puzzle --export ./out
    ...
    python mechanistic_empathy_engine.py -c autism -g social_sim -e ./out --engine arcade
...
__version__ = "1.2.0"
__author__  = "The Education Singularity / Open Source Contributors"
__license__ = "MIT"
...
class StaminaSystem:
    """
    Regenerative stamina whose rate, cap, and drain are all driven by
    the condition's serotonin (and supporting NT) baseline.

    Biological grounding
    --------------------
    Serotonin modulates energy homeostasis via the raphe nuclei →
    hypothalamus → HPA axis pathway.  Low serotonin = impaired sleep
    architecture, blunted reward from rest, and chronically elevated
    cortisol — all of which reduce how fast and how fully stamina can
    recover.

    Parameters (all derived automatically from NT profile)
    -------------------------------------------------------
...
"""
# NOTE FOR FLIT: Flit reads mechanistic_empathy_engine.py, not this file.
...
# ── MechanicRuntime ───────────────────────────────────────────────────────────
# Every GameMechanic from the engine spec is registered here.
# Each one maps to a live modifier function that runs every frame.
MECHANICS = {{
{mechanic_rows}
}}
...
class MechanicRuntime:
    """
    Executes GameMechanic specs as real runtime behaviour.
    Each mechanic modifies the game_state dict that everything reads from.
    """
    ...
    def tick(self, gs: dict, dt: float) -> dict:
        ...
        # ── Base params from NT (always active) ───────────────────────────────
        ...
        # ── Stamina regen/drain (serotonin-driven) ────────────────────────────
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self, gs: dict, dt: float):
        ...
        # Colour reflects saturation (anhedonia / mood)
        ...
        # Update resting state for stamina
...
def build_level(condition: str) -> pygame.sprite.Group:
    """
    Generates platforms appropriate for the condition.
    Depression → wide, sparse, heavy. ADHD → many small scattered.
    Autism → ordered grid. Anxiety → no safe zones far apart. etc.
    """
    platforms = pygame.sprite.Group()

    # Floor — always present, full width
...
def apply_saturation(surface: pygame.Surface, sat: float) -> pygame.Surface:
    """Desaturate a surface — maps serotonin level to visual greyscale."""
    if sat >= 1.0:
        return surface
    sat = max(0.0, sat)
...
def main():
    ...
    while True:
        ...
        # ── Render ────────────────────────────────────────────────────────────
        # Base background colour — shifts with mood/saturation
...
"""
__version__ = "{__version__}"
# Pygame Zero needs no import — pgzrun injects everything.
